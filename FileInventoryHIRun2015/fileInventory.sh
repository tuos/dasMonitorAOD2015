...
# Perform Inventory
/usr/local/lio/bin/lio_du -h -s @:/cms/store/hidata/HIRun2015/ > store.inv.$COUNTER
STORE_RESULT=$?
echo "$(date) - performed inventory of /cms/store/hidata/HIRun2015/ with exit code $STORE_RESULT" >> fileInventory.log
# retry on 2 line output or error code twice
if [[ $( wc -l < store.inv.$COUNTER ) -lt 3 || $STORE_RESULT -ne 0 ]]; then
  sleep 300
  /usr/local/lio/bin/lio_du -h -s @:/cms/store/hidata/HIRun2015/ > store.inv.$COUNTER
  STORE_RESULT=$?
  echo "$(date) - Retry 1: /cms/store/hidata/HIRun2015/ with exit code $STORE_RESULT" >> fileInventory.log
fi
if [[ $( wc -l < store.inv.$COUNTER ) -lt 3 || $STORE_RESULT -ne 0 ]]; then
  sleep 300
  /usr/local/lio/bin/lio_du -h -s @:/cms/store/hidata/HIRun2015/ > store.inv.$COUNTER
  STORE_RESULT=$?
  echo "$(date) - Retry 2: /cms/store/hidata/HIRun2015/ with exit code $STORE_RESULT" >> fileInventory.log
fi
